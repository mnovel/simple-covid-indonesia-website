...
<body class="bg-light">
                ...
                <div class="row row-cols-1 row-cols-md-2 g-4 mb-5 mt-3 d-none" id="<?= preg_replace('/ /i', '', $res['nama']) ?>">
                    ...
                    <div class="col">
                        <div class="card name-collapse">
                            <img src="data:image/jpege;base64,<?= $fetch[0][2] ?>" class="card-img-top" alt="ayah">
                            <div class="card-body">
                                <h5 class="text-muted mb-4">Vaksin <?= $fetch[0][5] == 1 ? 'Pertama' : 'Kedua' ?></h5>
                                <h6><?= date('d F Y', strtotime($fetch[0][3])) ?></h6>
                                <p class="card-text text-uppercase"><?= $fetch[0][4] ?>.</p>
                            </div>
                        </div>
                    </div>
                    <div class="col">
                        <div class="card name-collapse">
                            <img src="<?= $fetch[1][2] ?? null != null ?  'data:image/jpege;base64,' . $fetch[1][2]  : 'assets/img/certificate.jpg' ?>" class="card-img-top" alt="ayah">
                            <div class="card-body">
                                <div class="d-flex bd-highlight">
                                    <div class="w-100 bd-highlight">
                                        <h5 class="text-muted mb-4">Vaksin <?= $fetch[1][5] ?? null == 1 ? 'Pertama' : 'Kedua' ?></h5>
                                    </div>
                                    <div class="flex-shrink-1 bd-highlight <?= $fetch[1][5] ?? null != null ? 'd-none' : '' ?>">
                                        <i class="far fa-info-circle text-info" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="bottom" title="<h5>Informasi Sertifikat E-Sertifikat</h5><p>vaksin kedua Anda belum terbit, segera lakukan vaksinasi kedua Anda sesuai lokasi & jadwal yang tercantum pada kartu vaksin Anda.</p>"></i>
                                    </div>
                                </div>
                                <h6><?= date('d F Y', $fetch[1][3] ?? null != null ? strtotime($fetch[1][3]) : strtotime('+28 day', strtotime($fetch[0][3]))) ?></h6>
                                <p class="card-text text-uppercase"><?= $fetch[1][4] ?? $fetch[0][4] ?>.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <form action="upload.php" method="POST" enctype="multipart/form-data">
                    <div class="modal-body bg-white text-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="number" name="urut" id="urut" placeholder="1">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Tanggal Vaksin</label>
                            <input class="form-control" type="date" name="tgl" id="tgl" placeholder="1">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Tempat Vaksin</label>
                            <textarea class="form-control" name="tempat" id="tempat" rows="3" placeholder="KKP KELAS II PROBOLINGGO WILAYAH KERJA PASURUAN"></textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Sertivikat Vaksin</label>
                            <input class="form-control" type="file" name="sertif" id="sertif" placeholder="KKP KELAS II PROBOLINGGO WILAYAH KERJA PASURUAN">
                        </div>
                    </div>
    ...
    <div class="modal fade" id="editSertif" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content bg-blue text-white">
                <div class="modal-header ">
